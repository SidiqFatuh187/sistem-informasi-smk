...
<head>
    ...
    <title>Tambah Siswa — SMK Wira Cipta Karya</title>
            ...
            <header class="bg-[#f3f4f6] border-b border-ink/10">
                <div class="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
                    <div>
                        ...
                        <h1 class="font-bold text-xl">Tambah Siswa</h1>
                    </div>
                    <a href="{{ route('students.index') }}" class="bg-black text-white px-4 py-2 rounded-full text-sm font-semibold hover:bg-slate-800">
                        Kembali
                    </a>
                </div>
            </header>
                ...
                <div class="bg-[#f3f4f6] rounded-2xl border border-ink/10 shadow-sm p-6">
                    <form method="POST" action="{{ route('students.store') }}" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input id="nisn" name="nisn" value="{{ old('nisn') }}" required class="w-full rounded-xl border border-ink/15 bg-cream/60 px-4 py-3 text-sm focus:outline-none focus:border-navy">
                            @error('nisn')
                        ...
                        <div>
                            ...
                            <input id="nama" name="nama" value="{{ old('nama') }}" required class="w-full rounded-xl border border-ink/15 bg-cream/60 px-4 py-3 text-sm focus:outline-none focus:border-navy">
                            @error('nama')
                                <p class="mt-2 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex justify-end gap-3">
                            ...
                            <button type="submit" class="rounded-full bg-black px-5 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
