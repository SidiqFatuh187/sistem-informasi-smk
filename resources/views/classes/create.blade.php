...
<head>
    ...
    <title>Tambah Kelas — SMK Wira Cipta Karya</title>
            ...
            <header class="bg-[#f3f4f6] border-b border-ink/10">
                <div class="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
                    <div>
                        ...
                        <h1 class="font-bold text-xl">Tambah Kelas</h1>
                    </div>
                ...
                <div class="bg-[#f3f4f6] rounded-2xl border border-ink/10 shadow-sm p-6">
                    <form method="POST" action="{{ route('classes.store') }}" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input id="nama_kelas" name="nama_kelas" value="{{ old('nama_kelas') }}" required class="w-full rounded-xl border border-ink/15 bg-cream/60 px-4 py-3 text-sm focus:outline-none focus:border-navy">
                            @error('nama_kelas')
                        ...
                        <div>
                            ...
                            <select id="academic_year_id" name="academic_year_id" required class="w-full rounded-xl border border-ink/15 bg-cream/60 px-4 py-3 text-sm focus:outline-none focus:border-navy">
                                ...
                                @foreach ($academicYears as $academicYear)
                                    <option value="{{ $academicYear->id }}" {{ old('academic_year_id', $academicYear->is_active ? $academicYear->id : '') == $academicYear->id ? 'selected' : '' }}>{{ $academicYear->tahun }} - {{ $academicYear->semester }}{{ $academicYear->is_active ? ' (Aktif)' : '' }}</option>
                                @endforeach
                            </select>
